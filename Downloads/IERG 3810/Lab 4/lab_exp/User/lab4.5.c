...
/* Convert unsigned int to string and reverse it */
static int utoa10(unsigned int val, char* buf){
	int i = 0;
	int r, l;
	if (val == 0){
		buf[0] = '0';
...
/* Initialize PS/2 key at PC11 for EXTI-11 interrupt, IRQ#40 */
void ps2key_exitInit(void){
	...
	/* EXTI->RTSR |= 1<<0; */ /* TR0 rising edge enabled */
	NVIC->IP[40]=0x10; /* set priority of this interrupt */
	NVIC->ISER[1]|=1<<8; /* IRQ40, enable interrupt */
}
...
/* Interrupt handler for EXTI15_10 (PS/2 key press) */
void EXTI15_10_IRQHandler(void){
	uint8_t bit, start, data, parity, stop, ones;
	/* design */
	if(EXTI->PR & (1<<11)){ /* if PR is clear */
		...
		if(ps2_bitcount >= 11){
			...
			start  =  (f >> 0) & 1;
			data   =  (f >> 1) & 0xFF;     /* 8 data bits */
			parity =  (f >> 9) & 1;
			stop   =  (f >> 10) & 1;
				
			/* Validate start=0, stop=1, odd parity */
			ones = __builtin_popcount((unsigned)data) + parity;
		...
		EXTKernel->PR = 1<<11; /* clear exception pending bit */
